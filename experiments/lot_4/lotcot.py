...
def Logic_Reasoner(context, question, option, model):
    ...
    content_answer = f"{response}\nPlease output the symbol of the answer A/B/C/D: "
    response_symbol = LLM_response(content_answer, model)
    print(response_symbol)
...
def run(path, model):
    ...
    count = 0
    logging.basicConfig(filename='logiccot.log', level=logging.INFO, encoding='utf-8')
