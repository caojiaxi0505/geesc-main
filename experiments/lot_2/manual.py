...
LLM_model = ChatOpenAI(model_name="gpt-4", openai_api_key="")
dataset_path = ''

def LLM_response(content, model):
    answer = model.invoke(content).content
    return answer
...
In addition, the combination of two propositions can be represented by&, for example, sentence "A and B" can be represented by "A&B". 
